...
export function FounderStory() {
  ...
  return (
    <section id="story" className="scroll-mt-20 border-t border-hairline bg-surface-0 py-20 md:py-28">
      <Reveal className="max-w-2xl mx-auto px-6">
        <SectionHeader className="mb-10" title="Why I built it." />
        <div className="space-y-5 text-base md:text-lg leading-relaxed text-white/80">
          {paragraphs.map((p) => (
            <p key={p}>{p}</p>
          ))}
        </div>
        {FOUNDER_SIGNATURE && <p className="mt-8 font-heading uppercase text-primary">{FOUNDER_SIGNATURE}</p>}
      </Reveal>
    </section>
  );
}
